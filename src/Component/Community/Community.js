...
const PAGE_SIZE = 5;   // 한 번에 보여줄 게시글 수
...
// 실제로 스크롤되는 조상 엘리먼트 찾기 (없으면 window)
...
function Community() {
    ...
    useEffect(() => {
        ...
        const onScroll = () => {
            ...
            // 바닥 320px 전에 다음 묶음 로드
            if (top + viewH >= totalH - 320) {
                setVisibleCount((prev) =>
                    prev >= posts.length ? prev : prev + PAGE_SIZE
                );
            }
        };
    ...
    return (
        <div className="community-container" ref={containerRef}>
            <CommunityHeader
                loginUser={loginUser}
            ...
            {/* 피드 */}
            <div className="community-feed" onClick={() => setOpenMenuNum(null)}>
                {isLoading ? (
                    ...
                    posts.slice(0, visibleCount).map((post) => (
                        <CommunityPost
                            ...
                            isFollowing={followings.some(
                                (f) =>
                                    Number(f.fto) === Number(post.member?.num)
                            )}
                            isMenuOpen={openMenuNum === post.num}
                ...
                {posts.length > 0 &&
                    (visibleCount < posts.length ? (
                        <div className="community-more">불러오는 중...</div>
                    ) : (
            ...
            <div className="community-fabs">
                ...
                {openReplyNum === null && (
                    ...
                    >
                        ...
                        <span className="community-write-text">기록하기</span>
                    </button>
                )}
            </div>
